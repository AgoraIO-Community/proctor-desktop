...
import { RoomInfo, RoomState } from "../../../api/room";
...
export const RoomListItem: FC<RoomListItemProps> = ({
  ...
  const dateStr = useMemo(() => {
    return `${dayjs(data.startTime).format(Format)}-${dayjs(
      data.endTime
    ).format("HH:mm")}`;
  }, [data]);
  ...
  return (
    <div
      className={`room-item-card ${roomStateClassNames[roomState]} ${className}`}
    >
      <div className="info">
